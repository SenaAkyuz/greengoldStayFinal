...
describe('green-gold-widget', () => {
  it('updates preview overrides without emitting analytics, then restores saved config', async () => {
    installFetch();
    const el = await mountWidget({ ...baseAttrs(), 'data-preview': 'true', 'data-rooms': '2', 'data-preview-amount': '4', 'data-preview-co2': '10', 'data-preview-currency': 'EUR' });
    expect(el.shadowRoot!.querySelector('.row-sub')!.textContent).toContain('4');
    el.setAttribute('data-preview-amount', '7');
    await flush();
    expect(el.shadowRoot!.querySelector('.row-sub')!.textContent).toContain('7');
    expect(el.shadowRoot!.querySelector('.co2-note')!.textContent).toContain('2 oda');
    el.removeAttribute('data-preview-currency');
    await flush();
    expect(el.shadowRoot!.querySelector('.row-sub')!.textContent).toContain('3');
